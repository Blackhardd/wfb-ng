...
DRONE_OFFLINE_THRESHOLD = 50.0    # Используется для определения drone offline в других частях кода (не для hop)
CHANNEL_DEAD_GRACE_PERIOD = 5     # Период ожидания перед объявлением канала мёртвым

# --- Логика recovery (как в Status Manager: lost → recovery) ---
PACKET_TIMEOUT = 5.0              # Сек без пакетов → считаем "lost"
LOST_TO_RECOVERY_TIMEOUT = 10.0   # Сек в состоянии lost → переход в recovery (hop на reserve). Итого ~15 сек до hop
STABLE_LINK_PACKETS = 3           # Минимум пакетов для "стабильной" связи
STABLE_LINK_PERIOD = 1.0          # За период (сек) для учёта стабильной связи

# --- Настройки startup ---
STARTUP_GRACE_PERIOD = 20.0       # Период ожидания связи при старте

# После перехода на новый канал даём время на приём перед recovery (нет "No packets for never" сразу)
CHANNEL_RECOVERY_GRACE = 8.0      # Сек на канале без пакетов — только потом считаем "need recovery"

# Обычный hop только при активной связи: дрон должен успеть получить команду и переключиться вместе с ГС
NORMAL_HOP_MAX_PACKET_AGE = 2.0   # Сек: если последний пакет старше — обычный hop не делаем (только recovery)
...
class Channel():
    def __init__(self, freq):
        ...
        self._on_packet_received = None

    def _update_score(self):
        """Обновление score канала на основе PER и SNR"""
        rssi = self.get_last_rssi()
        ...
        log.msg(f"Channel {self._freq}{' MHz' if self._freq > 2000 else ''} - RSSI: {rssi_str}, PER: {per}%, SNR: {snr:.2f} dB, Score: {self._score[-1]:.2f}")
        ...
        if packet_info_parts:
            ...
            log.msg("RSSI: {}, PER: {}% {}".format(rssi_str, per, packet_info))
    ...
    def get_last_rssi(self):
        """Последний усреднённый RSSI по каналу (для PowerSelection). None если данных нет."""
        rssi_list = []
        for meas in self._measurements.values():
            if len(meas) > 0 and meas[-1].rssi != 0:
                rssi_list.append(meas[-1].rssi)
        return int(round(sum(rssi_list) / len(rssi_list))) if rssi_list else None
    ...
    def is_dead(self, grace_seconds=CHANNEL_DEAD_GRACE_PERIOD):
        """
        Канал считаем мёртвым только если на нём уже была связь и потом пропала (PER=100% дольше grace).
        При включении данных ещё не было — канал не мёртвый, просто ждём первого пакета.
        """
        if not self.has_received_data():
        ...
        # Защита: PER = 100% - канал мёртвый (неважно как долго offline дрон)
    ...
    def add_measurement(self, rx_id, stats):
        ...
        # Защита: Обновляем время последнего пакета если получили данные; уведомляем о приходе пакета для проверки стабильной связи
        if stats.p_total > 0:
    ...
    def set_on_packet_received(self, callback):
...
class Channels:
    def __init__(self, freqsel, channel_frequencies):
        ...
        for chan in self._list:
            ...
            chan.set_on_packet_received(self.freqsel.on_packet_received)
    ...
    def on_channel_score_updated(self, channel):
        ...
        Логика recovery (как в Status Manager):
        1. Нет пакетов > PACKET_TIMEOUT (5 сек) → считаем "lost", запоминаем _lost_since.
        2. В состоянии lost ещё LOST_TO_RECOVERY_TIMEOUT (10 сек) → recovery hop на reserve.
        3. Итого до hop: ~15 сек без пакетов (вместо одного таймаута 20 сек).
        4. Пакеты снова пришли → сбрасываем _lost_since и флаг recovery.
        """
        now = time.time()
        time_since_last_packet = None
        if channel._last_packet_time > 0:
            time_since_last_packet = now - channel._last_packet_time

        # Есть недавние пакеты — сбрасываем состояние "lost" (окно стабильной связи обновляется в on_packet_received)
        if time_since_last_packet is not None and time_since_last_packet <= PACKET_TIMEOUT:
            self.freqsel._lost_since = None
        else:
            # Нет пакетов дольше PACKET_TIMEOUT — входим в "lost"
            if time_since_last_packet is not None and time_since_last_packet > PACKET_TIMEOUT:
                if self.freqsel._lost_since is None:
                    self.freqsel._lost_since = now
                    log.msg(f"[Lost] No packets for {time_since_last_packet:.1f}s, lost state started")

        # ===================== ПРИОРИТЕТ 1: RECOVERY HOP (работает ВСЕГДА) =====================
        # После hop на новый канал у него нет истории пакетов — не считать это "no packets", дать время на приём
        time_on_channel = now - (channel._switched_at if channel._switched_at > 0 else self.freqsel._startup_time)
        need_recovery = False
        if time_since_last_packet is None:
            if not self.freqsel.is_in_startup_grace_period() and time_on_channel >= CHANNEL_RECOVERY_GRACE:
                need_recovery = True
        elif self.freqsel._lost_since is not None:
            time_in_lost = now - self.freqsel._lost_since
            if time_in_lost >= LOST_TO_RECOVERY_TIMEOUT:
                need_recovery = True

        if need_recovery:
            reserve_channel = self.freqsel.channels.reserve()
            if channel.freq() == reserve_channel.freq():
                return
            if not self.freqsel._is_scheduled_recovery_hop and not self.freqsel._is_scheduled_hop:
                time_str = f"{time_since_last_packet:.1f}s" if time_since_last_packet else "never"
                lost_str = f", {now - self.freqsel._lost_since:.1f}s in lost" if self.freqsel._lost_since else ""
                log.msg(f"No packets for {time_str}{lost_str}, switching to reserve channel {reserve_channel.freq()}")
            self.freqsel.schedule_recovery_hop()
            return

        # Связь восстановилась: сбрасываем recovery только при стабильной связи (3 пакета за 1 сек), иначе один шумный пакет не отменит hop
        if self.freqsel._is_scheduled_recovery_hop and self.freqsel.is_link_stable():
            log.msg(f"Link stable ({STABLE_LINK_PACKETS} packets in {STABLE_LINK_PERIOD}s), clearing recovery hop flag")
            self.freqsel._is_scheduled_recovery_hop = False
        if time_since_last_packet is not None and time_since_last_packet < PACKET_TIMEOUT:
            self.freqsel._lost_since = None
        
        # ===================== ПРИОРИТЕТ 2: ОБЫЧНЫЕ HOP (только по согласованию с дроном) =====================
        # Обычный hop = оба переключаются. Если пакетов нет — дрон не получит команду, не делаем обычный hop (только recovery).
        if time_since_last_packet is None or time_since_last_packet >= NORMAL_HOP_MAX_PACKET_AGE:
            return
        # В состоянии lost не делаем обычный hop — только recovery на reserve (приоритет 1)
        if self.freqsel._lost_since is not None:
            return
        # Обычный hop только если на этом канале реально была связь и ухудшилась (PER/score)
        if not channel.has_received_data():
            return
        # Защита: Не переключаем каналы (обычный hop) пока связь не была установлена хоть 1 раз
        ...
        time_on_channel = time.time() - (channel._switched_at if channel._switched_at > 0 else self.freqsel._startup_time)
        if time_on_channel < HOP_MIN_TIME_ON_CHANNEL:
            return

        # Защита: при отличной связи не переключаем (избегаем лишнего hop при PER 0%, Score 100)
        if channel.per() < 10 and channel.score() > 85:
            return
...
class FrequencySelection:
    def __init__(self, manager):
        ...
        self._lost_since = None           # Время входа в "lost" (нет пакетов > PACKET_TIMEOUT)
        self._recent_packet_times = []    # Временные метки пакетов для проверки стабильной связи
    ...
    def get_current_freq(self):
        """Текущая частота канала (MHz). None если каналов нет."""
        if not self.channels or self.channels.count() == 0:
            return None
        ch = self.channels.current()
        return ch.freq() if ch else None

    def get_current_rssi(self):
        """Текущий усреднённый RSSI по текущему каналу (для PowerSelection). None если данных нет."""
        if not self.channels or self.channels.count() == 0:
            return None
        ch = self.channels.current()
        return ch.get_last_rssi() if ch else None

    def get_current_per(self):
        """Текущий PER (%) по текущему каналу (для лога на дроне). None если данных нет."""
        if not self.channels or self.channels.count() == 0:
            return None
        ch = self.channels.current()
        return ch.per(SCORE_FRAMES) if ch else None

    def get_current_snr(self):
        """Текущий SNR (dB) по текущему каналу (для лога на дроне). None если данных нет."""
        if not self.channels or self.channels.count() == 0:
            return None
        ch = self.channels.current()
        return ch.snr(SCORE_FRAMES) if ch else None
    ...
    def on_packet_received(self):
        """Вызывается при каждом приходе пакетов (из Channel.add_measurement). Обновляет окно для is_link_stable()."""
        now = time.time()
        self._recent_packet_times.append(now)
        self._recent_packet_times = [t for t in self._recent_packet_times if now - t <= STABLE_LINK_PERIOD]

    def is_link_stable(self):
        """Стабильная связь: не менее STABLE_LINK_PACKETS пакетов за последние STABLE_LINK_PERIOD сек."""
        now = time.time()
        self._recent_packet_times = [t for t in self._recent_packet_times if now - t <= STABLE_LINK_PERIOD]
        return len(self._recent_packet_times) >= STABLE_LINK_PACKETS
    ...
    def hop(self):
        ...
        # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: ГС планирует hop локально СРАЗУ, не дожидаясь ответа от дрона
        # Проблема: дрон переключается раньше, чем ответ успевает вернуться через туннель
        # Решение: оба устройства переключаются синхронно по локальному времени
        ...
        log.msg(f"GS: Frequency hop scheduled locally in {delay:.2f} seconds")
        
        # Отправляем команду дрону (он тоже запланирует hop на то же время)
        d = self.manager.client_f.send_command({"command": "freq_sel_hop", "action_time": action_time})
        
        if d is None:
            log.msg("WARNING: send_command returned None, GS will hop alone (drone may not be reachable)")
            return
        
        # Логируем результат отправки команды, но не меняем план hop (он уже запланирован)
        d.addCallback(lambda res: log.msg(f"[HOP COMMAND SENT] Drone acknowledged: {res}"))
        d.addErrback(lambda err: log.msg(f"[HOP COMMAND FAILED] Drone did not acknowledge: {err}"))
    ...
    def do_hop(self, channel=None):
        ...
        # Защита 3: отмена recovery hop только при стабильной связи (избегаем отмены по одному шумному пакету)
        if self._is_scheduled_recovery_hop and self.is_link_stable():
            self._is_scheduled_recovery_hop = False
            log.msg("Recovery channel hop cancelled: link stable")
            return
        ...
        self._recent_packet_times = []  # стабильная связь считается только по новому каналу
